...
| [`bno055.py`](./bno055.py) | Adafruit BNO055 9-DoF absolute-orientation IMU (`sensor_type: "bno055_9dof"`). NDOF fusion; emits heading/roll/pitch (deg), calibration status, die temperature. **Node boot-config note:** the BNO055 stretches the I2C clock and the Pi's hardware I2C controller handles that badly — prefer a software bus (`dtoverlay=i2c-gpio,bus=3,i2c_gpio_sda=23,i2c_gpio_scl=24`) or slow the hardware bus (`dtparam=i2c_arm_baudrate=10000`) in `/boot/firmware/config.txt`. The driver also retries each read twice on OSError/timeout. |
...
2. **Add the ACS758 + ADS1115** in series with the live wire. Power the pump manually (bypass the relay), watch the I2C reading via `python -c "from agal_one_agent.sensors.ads1115 import Ads1115; import time; a = Ads1115(); [print(a.read_v(0)) for _ in range(50)] "`. Confirm voltage centres on ~2.5 V when off, deviates when on.
